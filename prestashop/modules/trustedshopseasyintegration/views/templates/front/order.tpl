...
{if !empty($confirmationDetails)}
  ...
  <div id="trustedShopsCheckout" style="display: none;" data-app="TrustedShopsApp">
    <span id="tsCheckoutOrderNr">{$confirmationDetails->getTsCheckoutOrderNr()|escape:'htmlall':'UTF-8'}</span>
    <span id="tsCheckoutBuyerEmail">{$confirmationDetails->getTsCheckoutBuyerEmail()|escape:'htmlall':'UTF-8'}</span>
    <span id="tsCheckoutOrderAmount">{$confirmationDetails->getTsCheckoutOrderAmount()|escape:'htmlall':'UTF-8'}</span>
    <span id="tsCheckoutOrderCurrency">{$confirmationDetails->getTsCheckoutOrderCurrency()|escape:'htmlall':'UTF-8'}</span>
    <span id="tsCheckoutOrderPaymentType">{$confirmationDetails->getTsCheckoutOrderPaymentType()|escape:'htmlall':'UTF-8'}</span>
    {if $confirmationDetails->isSendProducts() && !empty($confirmationDetails->getProducts())}
      <!-- product reviews start -->
      {foreach $confirmationDetails->getProducts() as $productItem}
        <span class="tsCheckoutProductItem">
          <span class="tsCheckoutProductUrl">{$productItem->getTsCheckoutProductUrl()|escape:'htmlall':'UTF-8'}</span>
          <span class="tsCheckoutProductImageUrl">{$productItem->getTsCheckoutProductImageUrl()|escape:'htmlall':'UTF-8'}</span>
          <span class="tsCheckoutProductName">{$productItem->getTsCheckoutProductName()|escape:'htmlall':'UTF-8'}</span>
          <span class="tsCheckoutProductSKU">{$productItem->getTsCheckoutProductSKU()|escape:'htmlall':'UTF-8'}</span>
          <span class="tsCheckoutProductGTIN">{$productItem->getTsCheckoutProductGTIN()|escape:'htmlall':'UTF-8'}</span>
          <span class="tsCheckoutProductMPN">{$productItem->getTsCheckoutProductMPN()|escape:'htmlall':'UTF-8'}</span>
          <span class="tsCheckoutProductBrand">{$productItem->getTsCheckoutProductBrand()|escape:'htmlall':'UTF-8'}</span>
        </span>
      {/foreach}
      <!-- product reviews end -->
    {/if}

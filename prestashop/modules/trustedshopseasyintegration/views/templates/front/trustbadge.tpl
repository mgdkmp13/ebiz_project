...
{if !empty($trustbadgeModels)}
    {foreach $trustbadgeModels as $trusbadgeModel}
        {if !empty($trusbadgeModel->getAttributes())}
          <!-- added by Trusted Shops app: Start -->
          <{$trusbadgeModel->getTag()|escape:'htmlall':'UTF-8'}
            {foreach $trusbadgeModel->getAttributes() as $attributeName => $value}
                {$attributeName|escape:'htmlall':'UTF-8'}="{$value|escape:'htmlall':'UTF-8'}"
            {/foreach}
           data-app="TrustedShopsApp"></{$trusbadgeModel->getTag()|escape:'htmlall':'UTF-8'}>
          <!-- End -->
        {/if}
    {/foreach}
{/if}

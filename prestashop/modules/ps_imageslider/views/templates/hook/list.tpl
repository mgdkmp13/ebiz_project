...
class Ps_ImageSlider extends Module implements WidgetInterface
{
    ...
    public function getSlides($active = null)
    {
        ...
        // Slajdy wyłączone (disabled)
        $slides2 = Db::getInstance((bool) _PS_USE_SQL_SLAVE_)->executeS(
            'SELECT hs.`id_homeslider_slides` as id_slide, hss.`position`, hss.`active`, hssl.`title`,
            hssl.`url`, hssl.`legend`, hssl.`description`, hssl.`image`
            FROM ' . _DB_PREFIX_ . 'homeslider hs
            LEFT JOIN ' . _DB_PREFIX_ . 'homeslider_slides hss ON (hs.id_homeslider_slides = hss.id_homeslider_slides)
            ...
            WHERE id_shop = ' . (int) $id_shop . '
            AND hssl.id_lang = ' . (int) $id_lang . '
            AND hssl.`image` <> "" AND hss.`active` = 0' . 
            ' ORDER BY hss.position'
        );
        ...
        // Zwrócenie obu zestawów slajdów
        return ['slides' => $slides, 'slides2' => $slides2];
    }
    ...
    {
        $slidesData = $this->getSlides();
        $slides = $slidesData['slides'];
        $slides2 = $slidesData['slides2'];

        foreach ($slides as $key => $slide) {
            $slides[$key]['status'] = $this->displayStatus($slide['id_slide'], $slide['active']);
            $associated_shop_ids = Ps_HomeSlide::getAssociatedIdsShop((int) $slide['id_slide']);
            $slides[$key]['is_shared'] = (count($associated_shop_ids) > 1);
        }

        foreach ($slides2 as $key => $slide) {
            $slides2[$key]['status'] = $this->displayStatus($slide['id_slide'], $slide['active']);
            $associated_shop_ids = Ps_HomeSlide::getAssociatedIdsShop((int) $slide['id_slide']);
            $slides2[$key]['is_shared'] = (count($associated_shop_ids) > 1);
        }
        ...
        $this->context->smarty->assign(
            [
                ...
                'slides' => $slides,
                'slides2' => $slides2,  // Przekazanie slajdów wyłączonych do szablonu

...
{
    require_once _PS_MODULE_DIR_ . 'trustedshopseasyintegration/src/Service/UpgradeService.php';
    require_once _PS_MODULE_DIR_ . 'trustedshopseasyintegration/src/API/Request/CustomRequest.php';
    require_once _PS_MODULE_DIR_ . 'trustedshopseasyintegration/src/API/Response/ArrayResponse.php';
    /** @var TrustedshopsAddon\Service\UpgradeService $upgradeService */
    $upgradeService = new TrustedshopsAddon\Service\UpgradeService();
    $isUpgradeDone = $upgradeService->updateNewStatus();

    $config = json_decode(Configuration::get($module::ORDER_STATUS_EVENTS_CONFIG), true);

    $configStatus = _PS_OS_SHIPPING_;

    $orderStatusDefaultCheckout = [
        'ID' => 'checkout',
        'name' => 'checkout',
        'event_type' => 'checkout',
    ];
    ...
    if (isset($config['all']) && isset($config['all']['trigger_order_status_id'])) {
        $configStatus = $config['all']['trigger_order_status_id'];

        $orderStatus = new OrderState((int) $configStatus, \Context::getContext()->language->id);

        $orderStatusSaved = [
            'ID' => (string) $orderStatus->id,
            'name' => (string) $orderStatus->name,
            'type' => 'order_status_from_prestashop',
        ];
    } else {
        $orderStatusSaved = $orderStatusDefaultCheckout;
    }

    $channelsConfig = [];
    /** @var ChannelService $channelService */
    $channelService = ServiceContainer::getInstance()->get(ChannelService::class);
    $channels = $channelService->getMappedChannels()->getMappedChannels();
    foreach ($channels as $oneChannel) {
        ...
        $orderStatusEvent = $channelService->getOrderStatusEventsForChannel($ref, $salesChannel);
        if ((bool) $orderStatusEvent->isOrderStatusEventsActivated() === true) {
            $channelsConfig[$ref] = [
                'product' => $orderStatusSaved,
                'service' => $orderStatusSaved,
            ];
        } else {
            $channelsConfig[$ref] = [
                'product' => $orderStatusDefaultCheckout,
                'service' => $orderStatusDefaultCheckout,
            ];
        }
    }

    return Configuration::updateValue($module::ORDER_STATUS_PRODUCTS_SERVICE, json_encode($channelsConfig))
        && $isUpgradeDone;
}
